...
class Graph:
    def __init__(self, matrix, vertices, edges):
        ...
        self.objects: List[Any[Edge, Vertex]] = self.edges + self.vertices
    ...
    @staticmethod
    def make_vertices(n, cx, cy, radius, vsize, width, color):
        vertices = []
        angle = 2 * math.pi / n
        for i in range(n):
            name = name_letter(i)
    ...
    def random(cls, n, cx, cy, radius, weights=(1, 10), fill_factor=0.4, vsize=15, width=1, color=(0, 0, 0)):
        ...
        edges = []
        vertices = Graph.make_vertices(n, cx, cy, radius, vsize, width, color)
        for i in range(n-1):
